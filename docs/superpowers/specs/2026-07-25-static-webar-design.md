...
- `useCameraStream`：请求后置摄像头、维护独立预览、恢复 iOS 播放、复制当前帧以及释放轨道。
...
4. 存在目标时直接启动 MindAR；不存在目标时保持实时预览并提示拍摄第一个参照物。
5. 用户点击拍照后，应用从实时视频复制当前帧，预览不暂停。
6. 应用立即显示“参照物已设定”Toast，并开始压缩、持久化及重新编译。
7. 编译期间预览、留言查看和目标管理保持可用；拍照按钮暂时禁用以阻止并发编译。
8. 编译成功后执行受控重建式热更新。
